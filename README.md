...
- The validator list defaults to bonded, non-jailed validators. The selector uses `/validators?status=inactive` and `/api/validators?status=inactive` for jailed and non-bonded validators, following upstream pagination and retaining profile identities, SSR and 30-second refreshes. Lists render 20 validators per page (`&page=2` for the next inactive page); only that page's identities mount and poll, and SSR preloads only its profiles. The shared active directory and default API response remain limited to the active set.
- The galaxy background canvas is fixed to the viewport, capped at 30 frames per second, paused in hidden tabs and static for reduced-motion preferences. Fallback validator avatars are memoized so profile updates do not repeatedly regenerate every avatar.
